...
RegisterCommand("carrera", function(source, args)
    if args[1] == "limpiar" or args[1] == "abandonar" then
    ...
    elseif args[1] == "empezar" then
        ...
        if amount then
            ...
            startDelay = startDelay and startDelay*1000 or config_carreras_cl.joinDuration
...
AddEventHandler("StreetRaces:loadRace_cl", function(checkpoints)
    ...
    for index, checkpoint in pairs(recordedCheckpoints) do
        ...
        SetBlipColour(checkpoint.blip, config_carreras_cl.checkpointBlipColor)
    ...
    SetBlipRouteColour(checkpoints[1].blip, config_carreras_cl.checkpointBlipColor)
end)
        ...
        if IsPedInAnyVehicle(player, false) then
            ...
            if raceStatus.state == RACE_STATE_RACING then
                ...
                if raceStatus.checkpoint == 0 then
                    ...
                    if config_carreras_cl.checkpointRadius > 0 then
                        local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                        checkpoint.checkpoint = CreateCheckpoint(checkpointType, checkpoint.coords.x,  checkpoint.coords.y, checkpoint.coords.z, 
                            0, 0, 0, config_carreras_cl.checkpointRadius, config_carreras_cl.checkpointColor.r, config_carreras_cl.checkpointColor.g, config_carreras_cl.checkpointColor.b, 
                            config_carreras_cl.checkpointColor.a, 0)
                        SetCheckpointCylinderHeight(checkpoint.checkpoint, config_carreras_cl.checkpointHeight, config_carreras_cl.checkpointHeight, config_carreras_cl.checkpointRadius)
                    end

                    -- Set blip route for navigation
                    SetBlipRoute(checkpoint.blip, true)
                    SetBlipRouteColour(checkpoint.blip, config_carreras_cl.checkpointBlipColor)
                else
                    -- Check player distance from current checkpoint
                    local checkpoint = race.checkpoints[raceStatus.checkpoint]
                    if GetDistanceBetweenCoords(position.x, position.y, position.z, checkpoint.coords.x, checkpoint.coords.y, 0, false) < config_carreras_cl.checkpointProximity then
                        ...
                        if raceStatus.checkpoint == #(race.checkpoints) then
                            ...
                            if config_carreras_cl.checkpointRadius > 0 then
                                local checkpointType = raceStatus.checkpoint < #race.checkpoints and RACE_CHECKPOINT_TYPE or RACE_CHECKPOINT_FINISH_TYPE
                                nextCheckpoint.checkpoint = CreateCheckpoint(checkpointType, nextCheckpoint.coords.x,  nextCheckpoint.coords.y, nextCheckpoint.coords.z, 
                                    0, 0, 0, config_carreras_cl.checkpointRadius, config_carreras_cl.checkpointColor.r, config_carreras_cl.checkpointColor.g, config_carreras_cl.checkpointColor.b, 
                                    config_carreras_cl.checkpointColor.a, 0)
                                SetCheckpointCylinderHeight(nextCheckpoint.checkpoint, config_carreras_cl.checkpointHeight, config_carreras_cl.checkpointHeight, config_carreras_cl.checkpointRadius)
                            end

                            -- Set blip route for navigation
                            SetBlipRoute(nextCheckpoint.blip, true)
                            SetBlipRouteColour(nextCheckpoint.blip, config_carreras_cl.checkpointBlipColor)
                        end
                    end
                end

                -- Draw HUD when it's enabled
                if config_carreras_cl.hudEnabled then
                    ...
                    ESX.ShowHelpNotification(("~w~CHECKPOINT %d/%d (%dm) - ~w~%02d:%06.3f"):format(raceStatus.checkpoint, #race.checkpoints, checkpointDist, timeMinutes, timeSeconds))
                end
            ...
            elseif raceStatus.state == RACE_STATE_JOINED then
                ...
                elseif count <= config_carreras_cl.freezeDuration then
                    -- Display countdown text and freeze vehicle position
                    Draw2DText2(0.072, 0.65, ("%d"):format(math.ceil(count/1000.0)), 2.2)
                    ...
                    ESX.ShowHelpNotification("Carrera por ~g~"..race.amount.."~w~ empezando en ~w~"..math.ceil(count/1000.0).."~w~s - Unido")
                end
            -- Player is not in a race
                ...
                for index, race in pairs(races) do
                    ...
                    if proximity < config_carreras_cl.joinProximity and currentTime < race.startTime then
                        -- Draw 3D text
                        ...
                        ESX.ShowHelpNotification(("Carrera por ~g~$%d~w~ empezando en ~w~%d~w~s - Pulsa [~g~E~w~] para unirte"):format(race.amount, count))

                        -- Check if player enters the race and send join event to server
                        if IsControlJustReleased(1, config_carreras_cl.joinKeybind) then
        ...
        if raceStatus.state == RACE_STATE_RECORDING then
            ...
            if IsWaypointActive() then
                ...
                if (coords ~= nil) then
                    ...
                    SetBlipColour(blip, config_carreras_cl.checkpointBlipColor)
...
TriggerEvent('chat:addSuggestion', '/carrera', 'crear, limpiar, abandonar, guardar, borrar, lista, empezar, cancelar.')
TriggerEvent('chat:addSuggestion', '/carrera crear', 'Empieza la creación de los checkpoints de la carrera en el mapa.')
TriggerEvent('chat:addSuggestion', '/carrera limpiar', 'Limpia el mapa de checkpoints de la carrera y cancela la creación.')
TriggerEvent('chat:addSuggestion', '/carrera abandonar', 'Cancelar inscripción a la carrera.')
TriggerEvent('chat:addSuggestion', '/carrera guardar', 'Guarda la carrera.', {
    { name="nombre", help="Nombre de la carrera" }
})
TriggerEvent('chat:addSuggestion', '/carrera borrar', 'Borra una carrera guardada.', {
    { name="nombre", help="Nombre de la carrera" }
})
TriggerEvent('chat:addSuggestion', '/carrera lista', 'Muestra una lista de las carreras guardadas.')
TriggerEvent('chat:addSuggestion', '/carrera empezar', 'Empieza la carrera.', {
    { name="apuesta", help="Cantidad a apostar para unirse a la carrera." },
    { name="tiempo", help="Tiempo hasta empezar la carrera y apuntarse." }
})
TriggerEvent('chat:addSuggestion', '/carrera cancelar', 'Cancelar la carrera.')
